...
class PololuDriver(ServoDriver):
    ...
    def reset_servos(self):
        self.write(bytes([0xA2]))

    def set_position(self, servo, value):
        value *= 4
        posLSB = value & 0x7F
        posMSB = (value >> 7) & 0x7F
        self.write(bytes([0x84])+bytes([servo])+bytes([posLSB])+bytes([posMSB]))

    def set_speed(self, servo, value):
        speedLSB = value & 0x7F
        speedMSB = (value >> 7) & 0x7F
        self.write(bytes([0x87])+bytes([servo])+bytes([speedLSB])+bytes([speedMSB]))

    def set_acceleration(self, servo, value):
        accLSB = value & 0x7F
        accMSB = (value >>7) & 0x7F
        self.write(bytes([0x89])+bytes([servo])+bytes([accLSB])+bytes([accMSB]))

    def get_moving_state(self):
        self.write(bytes([0x93]))
        data = self.__serial_connection.read(1)
        if data:
            return ord(data[0])
        else:
            return None
    ...
    def trigger_script(self, number):
        self.write(bytes([0xA7])+bytes([number]))

    def stop_script(self):
        self.write(bytes([0xA4]))

    def get_script_state(self):
        self.write(bytes([0xAE]))
        data = self.__serial_connection.read(1)
        if data:
            return ord(data[0])
        else:
    ...
    def get_errors(self):
        self.write(bytes([0xA1]))
        data = self.__serial_connection.read(2)
